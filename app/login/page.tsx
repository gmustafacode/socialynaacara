        ...
        <div className="flex min-h-screen w-full items-center justify-center bg-black bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-neutral-900 via-black to-black">
            <Card className="w-full max-w-md border-white/10 bg-neutral-950 text-white shadow-2xl animate-in fade-in zoom-in duration-500">
                <CardHeader className="space-y-1 text-center">
                    ...
                    <CardTitle className="text-2xl font-bold tracking-tight">Socialyncara</CardTitle>
                ...
                <CardContent className="grid gap-4">
                    {showResend && (
                        <div className="p-3 rounded-lg bg-blue-500/10 border border-blue-500/20 text-xs text-blue-400 animate-in slide-in-from-top-2 duration-300">
                            Didn't get the email?
                            <button
                                onClick={handleResendEmail}
                                disabled={isResending}
                                className="ml-2 font-bold underline hover:text-blue-300 disabled:opacity-50"
                            >
                    ...
                    <Button variant="outline" type="button" disabled={isLoading} onClick={handleGoogleLogin} className="w-full border-white/10 bg-white/5 hover:bg-white/10 text-white hover:text-white">
                        {isLoading ? (
                    ...
                    <Button variant="outline" type="button" disabled={isLoading} onClick={handleGitHubLogin} className="w-full border-white/10 bg-white/5 hover:bg-white/10 text-white hover:text-white">
                        {isLoading ? (
                            <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                        ) : (
                            <svg className="mr-2 h-4 w-4" aria-hidden="true" fill="currentColor" viewBox="0 0 24 24">
